...
public class PontoGr extends Ponto implements IElemento {
    ...
    public JSONObject toJSON(){
        ...
        obj.put("x", (this.getX() / DimensionView.LARGURA));
        obj.put("y", (this.getY() / DimensionView.ALTURA));
        obj.put("id", this.getNomePto());
        obj.put("esp", this.getDiametro());
        JSONObject cor = new JSONObject();
        cor.put("r", this.getCorPto().getRed());
        cor.put("g", this.getCorPto().getGreen());
        cor.put("b", this.getCorPto().getBlue());
        obj.put("cor", cor);
    ...
    public boolean contem(int x, int y){
        int raioSelect = 5; // tolerância
        return Math.abs(this.getX() - x) <= raioSelect && Math.abs(this.getY() - y) <= raioSelect;
    }
}
